...
2026-07-21 노션 2025 대비 재검토 (docs/plans/notion-gap-analysis-2026-07-21.md): 포함/차기/제외
구분은 전부 유지. 단 "포함" 중 어느 Phase에도 배정돼 있지 않던 5건을 회수 배정 — 버전 히스토리 →
Phase 9(스냅샷 방식), 템플릿 → Phase 9 말미(내장 프리셋), 백링크 → Phase 11, 파일 업로드 →
Phase 9(이미지 블록과 동시), 댓글+멘션 → 보류 전환(1인 제품에서 셀프 코멘트 가치 낮음 — "오리와의
인라인 대화"로 재해석하는 안을 Phase 10 착수 게이트에서 재검토). Phase 9는 "블록 에디터"에서
"워크스페이스 코어"(앱 셸+페이지 계층+에디터+검색+내보내기+파일 업로드+Quick Capture 연결)로
범위 재정의.
...
2. ~~Claude Code 로그 경로/포맷 - Phase 5~~ -> 해소(2026-07-20, Phase 5 착수 전 실측): 세션 로그는
   `~/.claude/projects/<slug>/<session-uuid>.jsonl`에 저장된다. `<slug>`는 작업 디렉터리 절대경로를
   구분자/콜론을 `-`로 치환한 문자열(예: `c--Users-JeongHun-Desktop-Study-little-dev-duck-little-dev-duck`).
   파일은 JSON Lines - 줄마다 `type`(예: `queue-operation`, 대화 턴 등), `timestamp`(ISO 8601),
   `sessionId` 등을 포함하는 완전한 원문 트랜스크립트(툴 호출/훅 출력 포함, 세션당 수 MB). Rust
   수집기는 이 파일들을 스캔해 날짜별 세션 존재/작업 디렉터리(프로젝트 식별)만 집계하면 되고,
   원문 내용은 로컬에만 남기고 업로드하지 않는다(DECISIONS.md 6절 원칙과 일치).
...
11. ~~Tauri WebView 탑재 방식(정적 export vs 대안) - Phase 5~~ -> 해소(2026-07-20, Phase 5 착수 전
    사고 게이트): 원 설계(섹션 1)는 "Next.js static export"를 전제했으나, Phase 2~4에서 생긴
    Middleware 인증 게이트(`apps/web/src/proxy.ts`), 서버 컴포넌트(`await createClient()`,
    `redirect()`), API Route(`/api/github/contributions`, `/auth/callback`, `/auth/logout`)가
    전부 정적 export가 지원하지 않는 서버 로직에 의존하면서 원 전제가 깨졌다. 검토한 대안
    A(배포된 웹 URL을 WebView가 로드)/B(Node 서버를 Tauri sidecar로 로컬 기동)/C(정적 유지 +
    로그인 필요 기능은 웹 링크로 우회) 중 **A로 확정**(사용자 승인) - 서버 로직 100% 재사용,
    버전 스큐 없음, 구현량 최소, 무료 티어·최소 구현 원칙과 부합. 단점(완전 오프라인 불가)은
    수용 - Supabase 의존으로 원래도 온라인 전제였음. ARCHITECTURE.md 1절 갱신 완료, phase_05.md
    T1(Tauri 스캐폴딩)은 이 결정에 따라 WebView가 Vercel 배포 URL을 가리키도록 진행.
    **보안 리뷰 추가 사실(2026-07-21, T2 완료 후)**: 옵션 A 구조(`frontendDist`/`devUrl`을 배포 URL
    그 자체로 지정)에서는 Tauri가 이 origin을 "Local"(로컬 앱 자체)로 판정한다(설치된
    tauri 2.11.5 소스로 확인) — 그래서 `capabilities/*.json`의 origin별 스코핑은 이 구조에서
    추가 방어선이 되지 못하고, `security.csp` 설정도 `https://` 원격 콘텐츠에는 주입되지 않아
    무효다. 옵션 A를 유지하는 한 이건 구조적 특성이라 되돌리지 않고, 대신 `apps/web/src/proxy.ts`에
    실제 CSP+보안 헤더를 추가해 방어선을 옮겼다(docs/plans/phase_05.md T2절 상세). 다음에 Tauri
    쪽에 새 커맨드나 플러그인을 추가할 때는 "capability의 origin 스코핑을 믿지 말 것" — 최소
    권한은 애초에 필요한 플러그인/커맨드 자체를 최소로 유지하는 것으로 달성해야 한다.
12. **확정 스택 변경 — Auth provider에 Email(이메일+비밀번호) 추가** (2026-07-26, Phase 41 T1)
    - **무엇을**: CLAUDE.md 2절이 `Supabase Auth (Auth Google+GitHub)`로 못박아 둔 인증 방식에
      **이메일+비밀번호를 더한다.** 기존 Google·GitHub 동작은 한 글자도 바뀌지 않는다.
    - **왜**: **사용자 요청 0번**(2차 피드백 `docs/feedback-2026-07-26-2.md`의 "0. 로그인 화면" →
      확인 질문에 "이메일+비밀번호 로그인 추가"로 답). 우선순위 규칙(사용자 지시 > 확정 계약)에
      따라 기존 결정을 뒤집는다.
    - **뒤집는 것을 명시한다**: [Phase 40](plans/phase_40.md)의 "하지 않을 것" 첫 항목이 정확히
      이것을 거부했다 — "테스트 편의로 인증 방식을 늘리지 않는다". **그 판단은 그 맥락에서
      옳았다**(그때 근거는 테스트 편의뿐이었다). 지금은 사용자 요청이 근거이고, 테스트 편의
      (죽어 있는 e2e 44건 해금)는 **목적이 아니라 부수 효과**다.
    - **비용(정직하게)**: ① 인증 표면 +1 — 비밀번호 추측·재사용 공격 대상이 생긴다(OAuth만
      있을 때는 없던 표면이다) ② 비밀번호 재설정 경로가 **없으면 사용자가 영구 잠긴다**
      (Phase 41 T3, 미착수) ③ 무료 티어 SMTP 발송 상한이 **아직 조사되지 않았다**
      (`CONSTRAINTS_FREE_TIER.md`에 항목 없음 — T3 착수 시 추가) ④ 같은 이메일의 OAuth 계정과
      이메일 계정을 어떻게 다룰지(병합·거부·별 계정)가 **Supabase 기본 동작에 맡겨져 있다**
      — 요청 6-3([Phase 49](plans/phase_49.md))이 그 일이고 거기서 확정한다.
    - **완화 조치(T1에 포함)**: 자격증명 오류를 **원인 구분 없이 한 문구**로 낸다
      (core `authErrorMessage` + 테스트 10건) — "없는 계정"과 "틀린 비밀번호"를 구분하면
      **어느 이메일이 가입돼 있는지 알려주는 통로**가 된다(계정 열거). `User already registered`도
      같은 문구로 보낸다.
    - **되돌리는 방법**: Supabase 대시보드 → Authentication → Providers → **Email을 끄면**
      기존 OAuth 로그인은 그대로 동작한다. 폼은 남지만 `Email logins are disabled`를 잡아
      "이메일 로그인이 아직 준비되지 않았습니다. Google 또는 GitHub로 계속해 주세요."를
      띄우므로 **꺼진 상태가 정체 모를 실패로 보이지 않는다.** 코드 롤백은 커밋 되돌리기.
    - **지금 상태**: provider가 **꺼져 있다**(사용자만 켤 수 있다). 즉 이 변경은 배포됐지만
      **켜지기 전까지 앱 동작은 안내 문구 하나 외에 바뀌지 않는다**(Phase 35 계정 삭제와 같은 방식).
