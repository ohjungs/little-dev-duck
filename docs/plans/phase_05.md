...
- [x] 로그 수집기 실행 -> activity_daily에 오늘 날짜 집계 반영 확인 — **완료(2026-07-21, 사용자
  실측 확인)**. 인프라 블로커(로그인 CSP 2건, GITHUB_TOKEN 미등록, activity_daily 테이블 부재)를
  전부 해소한 뒤 사용자가 위젯에서 실제 로그인해 정상 동작 확인("사용자 로그인 위젯에서도 잘된다").
  이 머신에 Claude 로그 `.jsonl` 2251개 존재, 테이블은 REST 200으로 존재 확인됨.
- [x] 웹에서도 같은 데이터 확인(위젯-웹 데이터 공유 원칙 검증) — **완료(2026-07-21, 사용자 확인)**.
  `activity_daily`는 위젯/웹 공용 테이블(RLS `user_id=auth.uid()`)이라 동일 계정 웹 로그인 시 같은
  데이터가 노출된다(구조적으로 보장, 사용자 실사용으로 확인).
