...
## 블로커 2 — "Next.js static export" 설계 전제가 현재 아키텍처와 충돌 (사고 게이트 필요)
...
옵션 A가 가장 실용적으로 보이나(무료 티어 원칙, 최소 구현 원칙과 부합), ARCHITECTURE.md의 명시적
문구를 뒤집는 결정이라 DECISIONS.md 갱신 + 사용자 확정이 필요하다. **다음 세션/사용자 응답에서
확정 후 진행.**
...
### T0. 블로커 해소 (직렬, 계약 잠금 전 필수)
- [ ] Rust 툴체인 설치 확인
- [ ] 정적 export vs 대안(A/B/C) 결정 - ARCHITECTURE.md 갱신

### T1. apps/desktop — Tauri 2 스캐폴딩 (계약 잠금)
- [ ] `pnpm create tauri-app` 또는 수동 스캐폴딩, 모노레포 워크스페이스 편입
- [ ] WebView 대상 URL/모드는 T0 결정에 따름
...
### T3. supabase/migrations — activity_daily 테이블
- [ ] `date, source(github|claude_code), count, user_id` + RLS(`user_id = auth.uid()`)
- [ ] down 스크립트 동반(CLAUDE.md 5장 원칙)
