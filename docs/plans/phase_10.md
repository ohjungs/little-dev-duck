...
- **`packages/core`**: `toolSchema`(name/description/parameters=JSON Schema, kind: readonly|mutating),
...
- [추정] Gemini 무료 티어의 function calling 지원 모델·한도(현재 `gemini-flash-latest` 사용). 실측 필요.
- [가정] 첫 어댑터 Google Calendar는 기존 Google 로그인에 scope 추가 동의로 확장 — Supabase Auth의
  provider scope 재동의 흐름 실측 필요(공급자 토큰 접근·갱신 포함).
