...
### T2. 홈 화면 연결 — 완료
- [x] apps/web 홈 위젯 대시보드에 Duck을 추가. r3f Canvas는 SSR 환경에서 렌더 불가하므로
      `next/dynamic`으로 `{ ssr: false }` 동적 임포트.

### T3. 검증 — 완료 (2026-07-20 종료 처리)
- [x] 전 패키지 build/lint/test 통과.
- [x] 실제 로그인 세션에서의 렌더링·클릭 반응 — 별도 세션에서 검증 완료된 것으로 간주하고 종료
      (사용자 지시, 2026-07-20 21:56). 이 세션은 DETECT 리뷰(docs/reviews/2026-07-20-phase3.md)만
      직접 수행.
