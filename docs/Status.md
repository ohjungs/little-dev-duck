...
- [ ] 이월: T3 Tauri sidecar WebSocket 중계·토큰보안·스로틀링(데스크톱), T2 실 hooks/JSONL 소스, T5
  스프라이트 에셋(절차 드로잉 대체), T7 더블클릭 로그 패널, 리소스 예산 자동 하향 실측.
...
- [ ] 이월: T3 클러스터링, T4 Gmail 발송·스케줄러, T5 발송 알림·아침 브리핑, T6 온보딩·폴더·음소거, T7 스크랩→노트.
...
- [ ] T6 리포트 수치 검증(캐시-원본 정합성): 통계 대시보드가 선행돼야 의미가 있어 이월(대시보드 자체는
  로드맵 이 Phase의 배정 항목 아님 — 노션 격차 후속).
- **Phase 12 상태: 배정 6항목 중 5개 완료(T1~T5), T6은 대시보드 선행이라 이월. 사실상 완료.**
...
- 검증: core 135 / api 189 tests + web build + eslint GREEN(리뷰 반영 후 재실행). **db push 필요
  ...
  드래그 이동 확인).
- [ ] 후속: 뷰 추가/삭제·board groupBy 변경 UI, select 옵션 색상 선택, 필터·정렬. [이월]
