...
- 검증: T1~T3은 커밋 edbb839로 배포(core 135 / api 186 tests + web build + eslint GREEN). T4 추가 후
  재검증. **db push 필요(pages_db_view)** + **사용자 실기 검증 필요**(페이지→데이터베이스 전환→표/보드
  에서 행 추가·속성 편집·드래그 이동 확인).
