...
> ## 📌 다음 세션은 여기서 시작한다 (2026-07-27 00:1x 기준)
>
...
>
> **코드로 이어서 할 것**: [Phase 42](plans/phase_42.md)(확정 결함 7건 — 상단 UI 깨짐·뉴스
> 0건·인사말 이름·메모 색·뽀모도로 소리·습관 잔디·velog). Phase 41에 남은 것은 **전부
> 사용자·설계 대기**다: T2는 설계 재검토(클라이언트가 Supabase를 직접 부르므로 서버 메모리
> 상한이 걸릴 자리가 없다), T5·T6은 사용자가 시크릿 2개를 등록해야 열린다.
> T4는 [Phase 40](plans/phase_40.md)이 이미 했다 — **다시 만들면 인벤토리 위반**이다.
>
