...
현재 Phase: **Phase 8(AI 1단계) 코드 구현 완료 — 배포(마이그레이션·키)·실호출 검증만 사용자 대기
(2026-07-21 밤, `/loop /next-step` 자율).** Phase 1~7 완료(Phase 7 마이그레이션 프로덕션 적용).
...
- [x] 저장 시 임베딩 배선: MemoWidget/TodoWidget 생성·수정·삭제 → reindexSource(빈 텍스트=삭제).
- [x] 통합·검증: core 88 / api 75 / ai 6 / mascot 5 tests, 전체 next build GREEN(/api/ai/* 라우트 확인).
- **남은 사용자 몫(아침)**: (1) `supabase db push`(embeddings 테이블+match_embeddings), (2) Vercel
  `GEMINI_API_KEY` 등록(무료 티어), (3) 실호출 검증(저장→인덱싱, 질문→RAG 답변). 적용 전엔 대화 패널만
  키 미설정 상태 — 기존 위젯·게임화는 정상.
