...
현재 Phase: 1 (코어 기반) — 코드 작성 전부 완료, **전 항목이 사용자의 외부 계정 설정 대기**
...
- [x] T6 CI/CD — GitHub Actions(.github/workflows/ci.yml: build+lint+test) 작성 완료, Vercel Analytics 연결 완료. **GitHub 원격 생성/push + Vercel 프로젝트 연결은 사용자 실행. Sentry는 계정 미생성으로 [미해결] 이월**
...
4. GitHub 원격 저장소 생성 + push, Vercel 프로젝트 연결, 환경변수 등록 (절차: docs/setup/deploy-setup.md)
