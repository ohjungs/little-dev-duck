...
> - **[99]/[102] 라이브 데모 한계**: 백필 로직은 core 단위테스트로 결정론 증명됨. 실제 화면
>   "10장" 시연은 테스트 계정에 오늘자(24시간) 기사가 있는 피드가 없어 미완(코드 결함 아님,
>   외부 RSS 발행 빈도 문제) — 테스트로 추가한 피드 4개는 삭제해 원복.
> - 검증: core/api/web 8개 패키지 빌드·린트·테스트 전부 GREEN(548 테스트). 실기 확인은
