...
> ## 📋 다음(예약) — [Phase 29 draft](plans/phase_29.md): 내보낸 백업이 실제로 백업인가
> 코드로 확인한 사실: **"내 데이터 내보내기"에 문서 본문이 한 글자도 안 들어간다**
> (`listPages` select에 `content` 없음 + 내보내기가 제목·아이콘만 담음). 캘린더 일정·습관
> 체크 기록도 빠지고, **되가져오기 경로가 아예 없다.** 착수 직전 재검증 후 시작.
