...
>
> **코드로 이어서 할 것**: [Phase 43](plans/phase_43.md)의 **T2(DB 전환 이관)** —
> 다만 "설정에서 전체 db화"의 해석이 갈려 계획이 `[해석 필요]`로 남겨 뒀다(되돌릴 수 없는
> 일괄 변환일 수 있어 추측으로 만들지 않는다). 오리 도구·`⋯` 메뉴 쪽만 먼저 할 수 있다.
> T1·T3은 2026-07-27에 끝냈다.
