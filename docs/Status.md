...
현재 Phase: **Phase 8(AI 1단계) 코드 구현 완료 + main push/Vercel 배포 완료 — 마이그레이션·키·실호출
...
- **남은 사용자 몫(아침)**: (1) `supabase db push`(embeddings 테이블+match_embeddings, 이미 pull 시 자동
  포함), (2) Vercel 환경변수 `GEMINI_API_KEY` 등록(무료 티어 키), (3) 오리 대화 패널에서 "기존 메모·할일
  인덱싱" 1회 클릭 후 질문 → RAG 답변 검증. 키 등록 전엔 대화 패널만 "키 미설정"(500) — 기존 위젯·게임화는 정상.
