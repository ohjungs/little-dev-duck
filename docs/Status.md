...
> ## ✅ 2026-07-30 `/loop-eng` — 기능 게이트 2차(판단 불필요분) + 승인 판정 fail-closed
> 직전 사이클에서 "라우트→기능 매핑은 제품 의도라 임의로 정하지 않겠다"고 미뤘는데, 그중
> **해석 여지가 없는 둘**은 처리하고 나머지만 남겼다.
>
> - **추가 게이트 2건(판단 아님)**: `news` 기능 설명이 곧 "RSS 구독과 요약"이라
>   `/api/news/collect`가 정확히 그 일이고, `github`은 "커밋 잔디: GitHub 기여도 위젯"이라
>   `/api/github/contributions`가 그 데이터를 준다. 둘 다 기능 이름이 라우트를 그대로 지목한다.
>   **남은 미정은 `/api/ai/write`·`/api/ai/standup`·`/api/ai/duck-line` 3개**(mv 114번).
> - **부수 효과 명시**: github 라우트는 잔디 위젯 외에 **오리 기분 신호**(useDuckMood)도 쓴다.
>   기능을 끄면 오리 기분이 조용히 중립으로 떨어진다 — `!res.ok`를 무시하도록 이미 만들어져
>   있어 화면은 깨지지 않는다. 사용자의 GitHub 데이터를 끄기로 했다면 오리도 그걸 읽지 않는 게
>   맞다는 판단이고, 숨기지 않고 라우트 주석·mv에 적었다.
> - **`requiresApproval`를 fail-closed로**: `kind === "mutating"` → `kind !== "readonly"`.
>   오늘은 enum이 2개라 결과가 같지만 **방향이 다르다** — 전자는 "mutating이 아니면 자동 실행"
>   이라 나중에 종류가 하나 늘면(외부 발송·파괴적 작업) 그 새 종류가 **승인 없이 실행**된다.
>   안전 게이트의 기본값은 "명시적으로 안전한 것만 자동, 나머지는 승인"이어야 한다. 타입에 없는
>   값을 억지로 넣어 승인을 요구하는지 검사로 못박았다.
> - **정직하게 남기는 한계**: 새로 막은 두 라우트는 **전용 테스트가 없어 정적 잠금으로만** 확인
>   된다(코드에 헬퍼 호출과 403이 있는지). 게이트 메커니즘 자체의 런타임 증명은 duck-chat
>   라우트의 403/200 양방향 테스트가 담당하고, 나머지는 같은 한 줄 패턴을 쓴다.
> - 검증: turbo lint+test **17/17 GREEN**(core 1366 / api 527 / web 570) + tsc +
>   `next build` 성공(4개 라우트 정상 등록). 화면 변경은 없어 스크린샷 없음(4-1의 "안 건드린
>   화면은 스킵"). 로그인 뒤 화면 확인은 여전히 e2e 세션 만료로 이월(mv 109번).
