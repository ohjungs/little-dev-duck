...
> **운영에서 지금 깨져 있는 제품 버그**다.
>
> - **① `tests.create_user`가 `profiles_guard_admin_columns`에 막힘**(10·20·30 전멸).
>   SECURITY DEFINER여도 트리거는 발화하고 그 시점 `is_admin()`은 false다. 시드 동안만 끈다.
> - **② 로컬 컨테이너에 public 테이블 grant 없음**(50). 마이그레이션에 grant 문이 없고,
>   운영의 권한은 Supabase 플랫폼이 프로젝트 생성 시 거는 default privileges에서 왔다.
>   → `20260801110000_public_schema_grants.sql`로 코드에 옮겨 적었다(운영에선 no-op).
>   **"이 마이그레이션만으로 새 프로젝트를 세우면 앱이 안 뜬다"는 구멍이었다** — 복구 경로 확보.
> - **③ `throws_ok` 3인자 오용**(30·40·60). 3인자는 `(sql, errcode, errmsg)`라 한국어 설명문이
>   기대 에러 메시지로 해석됐다. Postgres가 만드는 문구는 계약이 아니므로 검사하지 않는다
>   (반대로 우리가 `raise`로 쓴 P0001 문구는 계약이라 그대로 둔다).
> - **④ `award_xp`가 level 0을 써서 `duck_state_level_check` 위반 — 운영 실버그.**
>   `level = (xp + amount) / 100`은 정수 나눗셈이라 누적 xp 100 미만이면 0인데 제약은
>   `level >= 1`이다. 즉 **누적 XP 100 미만 사용자는 XP 적립이 통째로 실패**해 왔다(습관 체크·
>   뽀모도로 보상 전부, `applyXpAward`가 throw하므로 조용히 넘어가지도 않는다).
>   고치면서 산식을 core의 `deriveLevel`(삼각수 곡선)에 맞췄다 — DB가 core와 다른 산식을 따로
>   구현해 위젯 배지(DB `level`)와 게이지(core `levelProgress`)가 갈라지고 있었다.
>   운영 최대 xp가 26이라 실사용 영향 0. 회귀 테스트를 `40_`에 추가했다.
> - **⑤ `create_bare_user`가 `auth.users` 소유자가 아님**(1차 수정 후 드러남).
>   소유자는 `supabase_auth_admin`이고 pgTAP이 붙는 `postgres`는 Supabase에서 진짜 슈퍼유저가
>   아니다. 트리거를 끄는 대신 트리거가 만든 profiles 행을 지운다.
> - 부수: `00_helpers.sql`이 TAP 플랜을 안 내보내 파스 에러로 실패 집계되고 있었다(나머지가 다
>   통과해도 잡이 빨갛게 남는 구조).
>
> **운영 적용 완료**(2026-08-01, claude.ai Supabase 커넥터 — 2026-07-30·31과 같은 경로).
> 마이그레이션 2건 모두 원격 이력에 올랐다(`award_xp_level_floor`, `public_schema_grants`).
> - **실측**: `duck_level_for_xp` 경계값 12개(0/1/10/26/99/100/101/299/300/599/600/1000)가
>   core `deriveLevel`과 **전부 일치**(1,1,1,1,1,2,2,2,3,3,4,5). 함수 정의에 옛 나눗셈 산식이
>   남아 있지 않음을 `pg_get_functiondef`로 확인.
> - **기존 데이터 보정 불필요**: `duck_state` 4행 전부 저장된 `level`이 새 산식 결과와 같다
>   (xp 26/20/10/0 → 모두 1). 백필 없음, 데이터 변경 0.
> - **[확인됨] 이력 버전 드리프트**: 커넥터로 적용하면 원격 version이 적용 시각으로 찍혀
>   저장소 파일명(`20260801100000`)과 다르다(`20260801043434`). 이 저장소에 이미 있는 현상이다
>   (`drop_messenger`: 파일 `20260731120000` vs 원격 `20260731121331`). 나중에
>   `supabase db push`를 돌리면 두 파일이 다시 적용되지만 둘 다 멱등이라
>   (`create or replace` · `grant`) 무해하다.

> ## ✅ 2026-07-31 사용자 결정 일괄 처리 — 보안 마이그레이션 2건 **실서버 적용·실측 검증 완료**
> 미결정으로 쌓여 있던 항목에 사용자가 답했다. 답과 그로 생긴 작업의 단일 출처:
> [DECISIONS-2026-07-31.md](loop-eng/DECISIONS-2026-07-31.md).
>
> - **적용 2건**: `message_sender_immutable`(보낸이 위조로 방 목록을 통째로 못 열게 만드는 것
>   차단), `message_attachment_delete_owner`(방 멤버 아무나 남의 이미지를 지우던 것을 올린
>   사람으로 좁힘). 사용자가 access token 발급 화면까지 갔지만 **토큰은 불필요**했다 —
>   claude.ai Supabase 커넥터로 직접 적용(2026-07-30과 같은 경로).
> - **적용 전 조회로 미적용을 확증**했다(원격 목록이 `room_id_immutable`에서 끝나 있었다).
> - **실측**: 위조 UPDATE 2종 전부 차단(사유 문구까지 확인), **정상 경로는 통과**(기능 손실 0).
>   전부 되돌아가는 트랜잭션 안에서 확인해 데이터에 흔적을 남기지 않았다.
>   어드바이저 ERROR 0건 — 새 트리거 함수는 목록에 없다(SECURITY DEFINER를 안 쓴 선택의 효과).
> - **코드분**(커밋 `4569d51`): 로고 흰 원형 배지 · 대시보드 카드 끌어서 순서 변경(손잡이만
>   draggable + 화살표 키 경로 + 렌더 테스트 4건) · 취약점 강제 상향(`--prod` 0건).
> - **강제 불가 1건 기록**: `brace-expansion`은 5.x가 기본 내보내기를 객체로 바꿔 minimatch@3에서
>   eslint가 통째로 죽는다(실측). 취향이 아니라 호환 불가라 남긴다 — 개발 전용이라 `--prod` 대상
>   아님. 해소된 감사 면제 3건은 목록에서 비웠다.
> - 검증: tsc · 2,754 tests · lint 0 errors · build · 시크릿 검사 전부 통과.
>   **화면 확인은 못 했다** — 로고·대시보드 모두 로그인 뒤 화면이고 e2e 세션이 만료돼 있다.
...
>
> - **적용**: `profiles_admin_column_guard`(스스로 관리자 되기 차단),
>   `room_id_immutable`(초대 없이 남의 방 들어가기 차단). 트리거 3개 존재·활성 확인.
> - **"적용됐다"에서 멈추지 않고 실측했다**: 실제로 위조 UPDATE를 시도해 **3건 전부 차단**
>   (반환 사유까지 확인), **정상 경로 2건은 통과**. 데이터 무결성도 확인 — 고아 행 0건,
>   프로필 3·방 1·멤버 2·메시지 10 변동 없음.
> - **검증 중 내 실수 기록**: 첫 시도에 `room_members`가 "차단 안 됨"으로 나왔는데 트리거
>   결함이 아니라 **내 검사가 NULL 비교로 0행을 매칭**한 것이었다(오리 멤버의 `user_id`가
>   NULL). 데이터를 먼저 확인해 손상 없음을 확인하고 재검증했다. 검사도 검사가 필요하다.
> - **문서 정정(중요)**: 여러 문서가 "사용자 1명(본인=관리자)"을 전제로 위험도를 낮게 적었는데
>   **실제 프로필은 3개**(admin·admin·user)다. 이번에 막은 권한상승은 가정보다 **실제 노출에
>   가까웠다**.
> - **부작용 명시**: PENDING이 안내했던 "관리자 지정 SQL"이 이제 막힌다(SQL 편집기엔 JWT가
>   없어 `is_admin()`이 false). 의도된 동작이고, 트리거를 잠시 끄는 복구 절차를 적어 뒀다.
> - 어드바이저 재실행: ERROR 0건. **내 새 트리거 함수는 목록에 없다** — SECURITY DEFINER로
>   만들지 않은 선택이 노출 표면을 늘리지 않았음을 어드바이저가 확인해 줬다.
> - 전체 기록: [migration-applied-2026-07-30.md](loop-eng/migration-applied-2026-07-30.md).
>   **다음 세션 후보 1건 발견**(착수 안 함): `touch_room_on_message`가 트리거 함수인데
>   `authenticated`에 RPC로 노출돼 있다 — 위험은 낮지만 회수 한 줄로 줄일 수 있다.

> ## ✅ 2026-07-30 `/loop-eng` — 오프라인 배너 접근성 + 교훈·리뷰 기록 (묶음 마감)
> 감사 목록의 남은 항목이 저가치라, 오늘 9사이클에서 **실제로 겪은 것**을 저장소 규범대로
> 장기 기억에 넣는 사이클로 썼다(CLAUDE.md 4절 Review-Learning Loop).
>
> - **코드**: 오프라인 배너가 순수 div라 붉은 띠가 **눈으로만** 보였다 — 스크린리더 사용자는
>   연결이 끊긴 사실을 듣지 못했다(오프라인은 이후 저장이 실패한다는 뜻이라 알려야 한다).
>   `role="status"` 추가. **assertive를 쓰지 않은 이유**를 검사로 못박았다 — 배너가 계속 떠
>   있어 polite로 충분하고, 연결이 불안정하면 assertive가 읽던 문장을 매번 끊는다.
> - **감사 지적 1건 기각**: "PWA 절반 구현(서비스 워커 없음)" → 결함 아님. `OfflineIndicator`는
>   오프라인을 **알리기만** 하고 오프라인 동작을 약속하지 않는다. 서비스 워커는 새 기능이고
>   캐시 스테일로 앱을 깨뜨릴 위험이 있어 착수하지 않았다(ponytail YAGNI).
> - **교훈 기록 — 중복을 만들지 않았다**: 오늘 겪은 4건 중 **둘은 기존 교훈의 재발**이어서
>   새 항목을 만들지 않고 그 항목을 갱신했다. ① [[L-14]](빌드 신선도)에 **가드가 닿지 않는
>   변종**(직접 `next start` + 낡은 서버 점유) 추가·재발견 1회 기록 ② [[L-19]]("주석은 검사가
>   아니다")에 **인가 재발** 기록 — `canUseFeature` 주석이 약속한 계약을 API가 안 지켰고,
>   **L-19를 적어 둔 지 3일 뒤 같은 모양으로 재발**했다는 사실이 이 항목의 핵심 값이다.
> - **신규 교훈 2건**: **L-22 안전 게이트의 기본값 방향**(위험한 것 열거 대신 안전한 것만
>   열거 — 지금 동작이 맞는데도 결함이 될 준비가 된 상태는 버그 찾는 눈에 안 보인다),
>   **L-23 목록이 둘이면 짝을 검사로 묶어라**(폴백이 친절해서 누락이 안 드러난다 —
>   `approvalLabelCoverage`가 만든 다음 사이클에 내 실수를 잡아 값을 증명한 기록 포함).
> - **리뷰 스냅샷**: [2026-07-30-loop-eng-autonomous](reviews/2026-07-30-loop-eng-autonomous.md)
>   — 9사이클 전체(수정 14건 · **기각 3건** · **내가 낸 결함 2건** · 한계 4건). immutable.
> - 검증: turbo lint+test **17/17 GREEN**(core 1366 / api 527 / web 573).

> (이전 항목 154건은 2026-07-29 정리로 [History.md](History.md) "Status 이관 기록"으로 옮겼다 —
> Status는 현재 상태만 담는다. 이관은 삭제가 아니라 이동이다.)

현재 상태 (2026-07-29 갱신): **Phase 63까지 코드분 완료.** 메신저(50~59)는 핵심 사용 경로
포함 배포됐으나 **세부 확장은 사용자 방향 답변 대기로 보류**. Phase 61(데일리 브리핑)·
62(마감일 사용성)·63(취업 준비)은 당일 완결. 미착수 draft 버퍼 없음 — 다음 큰 방향은
