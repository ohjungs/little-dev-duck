...
const TOOL_PREFERENCE_GUARD =
  ...
  "답하지 말고 제공된 도구(function)를 사용해 처리하라. " +
  // 2026-07-26 : 오리 - 지시문 - 조회도구와자료지침충돌
  // 원래 문장은 "[사용자 자료]는 사실을 묻는 질문에만 참고한다"로 끝났다. 그런데
  // "이번 주 마감 뭐 있어?"는 **사실을 묻는 질문**이라, 목록을 조회하는 도구가 생긴 뒤에도
  // 모델이 자료 쪽으로 기울 여지가 남는다. 더 중요한 건 [사용자 자료]가 벡터 검색 상위 일부라
  // **전부가 아니라는 사실**이다 — 그걸로 "남은 할 일"을 세면 조용히 틀린 답이 나온다.
  "다만 목록을 조회하는 도구(예: listTodos)가 있는 질문이라면, [사용자 자료]에 비슷한 내용이 " +
  "보이더라도 **반드시 그 도구로 확인한 뒤** 답하라. [사용자 자료]는 검색으로 추려진 일부일 뿐 " +
  "전체 목록이 아니므로, 개수·목록·기한을 자료만 보고 단정하면 틀린다. " +
