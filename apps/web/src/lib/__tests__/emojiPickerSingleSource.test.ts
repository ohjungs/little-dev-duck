...
describe("이모지 피커 단일 출처", () => {
  it("최근 이모지 키는 EmojiPicker에만 있다", () => {
    ...
    const picker = readFileSync(resolveFromWebRoot("src/components/EmojiPicker.tsx"), "utf8");
    expect(picker).toContain("ldd:recent-emojis");
  });

  it("PageEditor에 IconPicker 구현이 남아 있지 않다 (두 벌 방지)", () => {
    const src = readFileSync(resolveFromWebRoot("src/components/PageEditor.tsx"), "utf8");
