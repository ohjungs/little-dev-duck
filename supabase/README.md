...
pomodoro_sessions, calendar_events)도 2026-07-21 사용자 승인 하에 `supabase db push`로 적용 완료**
(`migration list --linked` 결과 10개 전부 local==remote 확인). RLS는 각 마이그레이션에 정책까지 원자적
포함이라 교차 사용자 노출 위험 없음. 적용 후 아래 검증 체크리스트를 신규 4테이블로도 수행할 것.
